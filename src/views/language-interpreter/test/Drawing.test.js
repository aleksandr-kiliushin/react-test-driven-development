...
describe.skip("Drawing", () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(TurtleModule, "Turtle").mockReturnValue(<div id="turtle" />)
    jest.spyOn(window, "requestAnimationFrame").mockReturnValue(cancelToken)
    jest.spyOn(window, "cancelAnimationFrame")
    jest.spyOn(AnimatedLineModule, "AnimatedLine").mockReturnValue(<div id="animatedLine" />)
    jest.spyOn(StaticLinesModule, "StaticLines").mockReturnValue(<div id="staticLines" />)
  })
  ...
  const triggerRequestAnimationFrame = (time) => {
    act(() => {
      const lastCall = window.requestAnimationFrame.mock.calls.length - 1
      const frameFn = window.requestAnimationFrame.mock.calls[lastCall][0]
      frameFn(time)
    })
  }

  const triggerAnimationSequence = (times) => times.forEach(triggerRequestAnimationFrame)

  it("renders an svg inside div#viewport", () => {
    renderWithStore(<Drawing />, { script: { drawCommands: [] } })
    expect(container.querySelector("div#viewport > svg")).not.toBeNull()
  })

  it("sets a viewbox of +/- 300 in either axis and preserves aspect ratio", () => {
    renderWithStore(<Drawing />, { script: { drawCommands: [] } })
    expect(svg()).not.toBeNull()
    expect(svg().getAttribute("viewBox")).toEqual("-300 -300 600 600")
    expect(svg().getAttribute("preserveAspectRatio")).toEqual("xMidYMid slice")
  })

  it("renders a Turtle within the svg", () => {
    renderWithStore(<Drawing />)
    expect(container.querySelector("svg > div#turtle")).not.toBeNull()
  })

  it("passes the turtle x, y and angle as props to Turtle", () => {
    renderWithStore(<Drawing />)
    expect(TurtleModule.Turtle).toHaveBeenCalledWith({ x: 0, y: 0, angle: 0 }, {})
  })
  ...
  it("renders StaticLines within the svg", () => {
    ...
    renderWithStore(<Drawing />)
    expect(container.querySelector("svg > div#staticLines")).not.toBeNull()
  })
  ...
  describe("movement animation", () => {
    ...
    it("invokes requestAnimationFrame repeatedly until the duration is reached", () => {
      ...
      expect(window.requestAnimationFrame.mock.calls.length).toEqual(3)
    })
  })
  ...
  it("calls cancelAnimationFrame on reset", () => {
    ...
    renderWithStore(<Drawing />, { script: { drawCommands: [] } })
    expect(window.cancelAnimationFrame).toHaveBeenCalledWith(cancelToken)
  })
  ...
  describe("rotation animation", () => {
    ...
    it("rotates the turtle", () => {
      ...
      expect(TurtleModule.Turtle).toHaveBeenLastCalledWith({ x: 0, y: 0, angle: 90 }, expect.anything())
    })

    it("rotates part-way at a speed of 1s per 180 degrees", () => {
      triggerAnimationSequence([0, 250])
      expect(TurtleModule.Turtle).toHaveBeenLastCalledWith({ x: 0, y: 0, angle: 45 }, expect.anything())
    })
  ...
  it("animates the next command once rotation is complete", async () => {
    ...
    triggerAnimationSequence([0, 500, 0, 250])
    expect(TurtleModule.Turtle).toHaveBeenLastCalledWith({ x: 150, y: 100, angle: 90 }, expect.anything())
  })
  ...
  describe("skipping animation", () => {
    ...
    it("sets the turtle at the final position", () => {
      renderWithStore(<Drawing />, store)
      expect(TurtleModule.Turtle).toHaveBeenLastCalledWith({ x: 123, y: 234, angle: 180 }, expect.anything())
    })
  })
  ...
  describe("resetting", () => {
    it("resets Turtle position and angle to all-zeros", async () => {
      ...
      expect(TurtleModule.Turtle).toHaveBeenLastCalledWith({ x: 0, y: 0, angle: 0 }, expect.anything())
    })
  })
  ...
  describe("undo", () => {
    it("resets Turtle position to the store turtle position", () => {
      ...
      expect(TurtleModule.Turtle).toHaveBeenLastCalledWith({ x: 123, y: 234, angle: 90 }, expect.anything())
    })
  })
  ...
  describe("isAnimating", () => {
    it("initially does not have the isAnimating class set", () => {
      ...
      expect(container.querySelector("#viewport").className).not.toContain("isAnimating")
    })
    ...
    it("adds isAnimating class to viewport when animation begins", () => {
      ...
      triggerRequestAnimationFrame(0)
      expect(container.querySelector("#viewport").className).toContain("isAnimating")
    })
