...
describe.skip("Dialog", () => {
  ...
  const renderDialog = (props) =>
    render(<Dialog message={"Hello"} buttons={[]} onChoose={onChooseSpy} onClose={onCloseSpy} {...props} />)
  ...
  it("renders message in a paragraph element", () => {
    ...
    expect(container.querySelector("p").textContent).toEqual("This is a message")
  })

  it("renders a div with className dialogButtons inside dialog", () => {
    renderDialog()
    expect(container.querySelector("div.dialog > div.dialogButtons")).not.toBeNull()
  })

  it("renders button properties", () => {
    renderDialog({ buttons: [{ id: "yes", text: "Yes" }] })
    expect(container.querySelector("button").id).toEqual("yes")
    expect(container.querySelector("button").textContent).toEqual("Yes")
  })
  ...
  it("renders all buttons inside the dialogButtons div", () => {
    ...
    const buttons = Array.from(container.querySelectorAll("div.dialogButtons > button"))
    expect(buttons.length).toEqual(2)
    expect(buttons.map((button) => button.id)).toEqual(["yes", "no"])
  })
