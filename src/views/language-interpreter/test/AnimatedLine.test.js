...
describe.skip("AnimatedLine", () => {
  ...
  it("draws a line starting at the x1,y1 co-ordinate of the command being drawn", () => {
    renderSvg(<AnimatedLine commandToAnimate={horizontalLine} turtle={turtle} />)
    expect(line()).not.toBeNull()
    expect(line().getAttribute("x1")).toEqual(horizontalLine.x1.toString())
    expect(line().getAttribute("y1")).toEqual(horizontalLine.y1.toString())
  })

  it("draws a line ending at the current position of the turtle", () => {
    renderSvg(<AnimatedLine commandToAnimate={horizontalLine} turtle={{ x: 10, y: 20 }} />)
