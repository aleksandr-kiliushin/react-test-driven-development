...
describe.skip("MenuButtons", () => {
  let container, renderWithStore, click

  beforeEach(() => {
    ;({ container, renderWithStore, click } = createContainerWithStore())
    DialogModule.Dialog = jest.fn(() => <div id="dialog" />)
  })

  const makeDialogChoice = (button) => {
    const lastCall = DialogModule.Dialog.mock.calls[DialogModule.Dialog.mock.calls.length - 1]
  ...
  describe("reset button", () => {
    ...
    it("is disabled initially", () => {
      renderWithStore(<MenuButtons />)
      expect(button("Reset").hasAttribute("disabled")).toBeTruthy()
    })
    ...
    it("dispatches an action of RESET when clicked", () => {
      ...
      click(button("Reset"))
      return expectRedux(store).toDispatchAnAction().matching({ type: "RESET" })
    })
  })
  ...
  describe("undo button", () => {
    ...
    it("dispatches an action of UNDO when clicked", () => {
      ...
      click(button("Undo"))
      return expectRedux(store).toDispatchAnAction().matching({ type: "UNDO" })
    })
  })
  ...
  describe("redo button", () => {
    ...
    it("dispatches an action of REDO when clicked", () => {
      ...
      click(button("Undo"))
      click(button("Redo"))
      return expectRedux(store).toDispatchAnAction().matching({ type: "REDO" })
    })
  })
  ...
  describe("skip animation button", () => {
    ...
    it("is disabled if animations have already been skipped", () => {
      ...
      expect(button("Skip animation").hasAttribute("disabled")).toBeTruthy()
    })
    ...
    it("is enabled if animations have already been skipped", () => {
      ...
      expect(button("Skip animation").hasAttribute("disabled")).toBeFalsy()
    })
    ...
    it("dispatches a SKIP_ANIMATING action when clicked", () => {
      ...
      return expectRedux(store).toDispatchAnAction().matching({ type: "SKIP_ANIMATING" })
    })
  })
  ...
  describe("sharing button", () => {
    ...
    it("dispatches an action of START_SHARING when dialog onChoose prop is invoked with reset", () => {
      ...
      return expectRedux(store).toDispatchAnAction().matching({ type: "START_SHARING", reset: true })
    })
    ...
    it("dispatches an action of START_SHARING when dialog onChoose prop is invoked with share", () => {
      ...
      return expectRedux(store).toDispatchAnAction().matching({ type: "START_SHARING", reset: false })
    })
    ...
    it("dispatches an action of STOP_SHARING when stop sharing is clicked", async () => {
      ...
      return expectRedux(store).toDispatchAnAction().matching({ type: "STOP_SHARING" })
    })
  })
  ...
  describe("messages", () => {
    ...
    it("renders a message when watching has started", () => {
      ...
      expect(container.innerHTML).toContain("<p>You are now watching the session</p>")
    })
  })
})
