...
describe.skip("Prompt", () => {
  ...
  beforeEach(() => {
    ;({ container, renderWithStore, change, keyPress } = createContainerWithStore())
  })

  const renderInTableWithStore = (component) => renderWithStore(<table>{component}</table>)
  ...
  const textArea = () => container.querySelectorAll("tr")[0].childNodes[1].childNodes[0]
  ...
  it("sets the textarea text to initially have a height of 20", () => {
    renderInTableWithStore(<Prompt />)
    expect(textArea().getAttribute("style")).toEqual("height: 20px;")
  })
  ...
  describe("user enters an instruction", () => {
    it("dispatches an action with the updated edit line when the user hits enter on the text field", () => {
      ...
      return expectRedux(store).toDispatchAnAction().matching({ type: "SUBMIT_EDIT_LINE", text: line })
    })
    ...
    it("dispatches a START_ANIMATING event", () => {
      const store = renderInTableWithStore(<Prompt />)
      enterInstruction("forward 10\n")
      return expectRedux(store).toDispatchAnAction().matching({ type: "START_ANIMATING" })
    })
  })
  ...
  describe("prompt focus", () => {
    ...
    it("dispatches an action notifying that the prompt has focused", () => {
      const store = renderInTableWithStore(<Prompt />)
      store.dispatch({ type: "PROMPT_FOCUS_REQUEST" })
      return expectRedux(store).toDispatchAnAction().matching({ type: "PROMPT_HAS_FOCUSED" })
    })
  })
})
