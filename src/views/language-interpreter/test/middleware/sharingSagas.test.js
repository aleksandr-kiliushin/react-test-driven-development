...
describe.skip("duplicateForSharing", () => {
  let dispatch
  let store
  let next
  ...
  const callMiddleware = (action) => duplicateForSharing(store)(next)(action)
...
describe("sharingSaga", () => {
  ...
  describe("STOP_SHARING", () => {
    ...
    it("dispatches an action of STOPPED_SHARING", async () => {
      await startSharing()
      store.dispatch({ type: "STOP_SHARING" })
      return expectRedux(store).toDispatchAnAction().matching({ type: "STOPPED_SHARING" })
    })
  })
  ...
  describe("watching", () => {
    ...
    it("dispatches a STARTED_WATCHING action", async () => {
      await startWatching()
      return expectRedux(store).toDispatchAnAction().matching({ type: "STARTED_WATCHING" })
    })
    ...
    it("dispatches a STOPPED_WATCHING action when the connection is closed", async () => {
      ...
      return expectRedux(store).toDispatchAnAction().matching({ type: "STOPPED_WATCHING" })
    })
  })
})
