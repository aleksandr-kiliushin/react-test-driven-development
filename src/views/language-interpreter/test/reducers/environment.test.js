import { defaultState, environmentReducer as reducer } from "../../src/reducers/environment"

describe.skip("environmentReducer", () => {
  ...
  it("sets shouldAnimate to false when receiving a SKIP_ANIMATING action", () => {
    expect(reducer({ shouldAnimate: true }, { type: "SKIP_ANIMATING" })).toEqual({
      shouldAnimate: false,
    })
  })

  it("sets shouldAnimate to true when receiving a START_ANIMATING action", () => {
    expect(reducer({ shouldAnimate: false }, { type: "START_ANIMATING" })).toEqual({
      shouldAnimate: true,
    })
  })
