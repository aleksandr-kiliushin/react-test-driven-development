...
import { finishParsingList, parseCall, parseNextListValue, parseNextToken } from "./parseCall"
import { performAll } from "./perform"
import { isParameterReference, parameterValue } from "./values"
...
const parseTo = (state, token) => {
  ...
  if (currentInstruction.collectingParameters && token.text.startsWith(":")) {
    return {
      parameters: [...currentInstruction.parameters, token.text.substring(1).toLowerCase()],
    }
  }
  if (token.text === "end") {
